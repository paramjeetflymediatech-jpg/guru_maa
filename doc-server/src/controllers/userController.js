...
async function apiCreateUser(req, res) {
    ...
    if (!name || !email) {
        return handleErrorWithStickyData(req, res, '/admin/users/create', 'Name and Email are required.', req.body);
    }

    // Check if email already exists
    const existingUser = await findUserByEmail(email);
    if (existingUser) {
        return handleErrorWithStickyData(req, res, '/admin/users/create', 'Email already exists.', req.body);
    }
    ...
    try {
        ...
        if (!userData.password) {
            ...
            try {
                ...
                req.session.flash = { type: 'success', message: 'User created and invitation sent.' };
            } catch (emailErr) {
                console.error("Email Error:", emailErr);
                req.session.flash = { type: 'warning', message: 'User created but failed to send invitation email.' };
            }
        } else {
            req.session.flash = { type: 'success', message: 'User created with password.' };
        }
...
async function apiUpdateUser(req, res) {
    ...
    if (conflictUser && conflictUser._id.toString() !== id) {
        return handleErrorWithStickyData(req, res, `/admin/users/edit/${id}`, 'Email is already taken by another user.', req.body);
    }
...
async function handleSetupPassword(req, res) {
    ...
    req.session.flash = { type: 'success', message: 'Password set successfully! Please login.' };
